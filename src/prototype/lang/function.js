...
(function(GLOBAL) {
  ...
  **/
   ...
   *
   *  The returned Number object has now a method named "clear" to clear the
   *  timeout without calling `window.clearTimeout` in your script. 
   *
   *  You can also use the timer-variable to indicate if the timer was cleared/executed already.
   *
   *
   ...
   *
   *      var timer = showMsg.delay(10, "Waiting...");
   *      // -> Waits 10 seconds, then alerts "Waiting..."
   *    
   *      timer.clear();
   *      // -> clears the timeout. behaves like window.clearTimeout(timer)
   *
   *      var timer = showMsg.delay(1, "Hello world");
   *      console.log(timer);
   *      // -> any number generated by window.setTimeout
   *
   *      // a second later ...
   *      console.log(timer);
   *      // -> null
